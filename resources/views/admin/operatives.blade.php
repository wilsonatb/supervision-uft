                        ...
                        <td>
                            <div class="btn-group-sm text-center" role="group" aria-label="Basic example">
                              @if(Auth::user()->hasRole('admin'))
                                ...
                                <a href="{{ route('user.updateShowOperativeAdmin', $user->id) }}" class="btn btn-warning">Editar</a>
                              @else
                                
                                <a href="{{ route('user.updateShowOperative', $user->id) }}" class="btn btn-warning">Editar</a>
                              @endif
  ...
  <script>
    $("#example1 tbody tr").on("click", function(event){
        var valorId = $(this).find("td:first-child").html();  
          ...
          @if(Auth::user()->hasRole('admin'))
            var url = "{{ route('user.deleteOperativeAdmin', ['temp', 'admin']) }}";
          @else
            var url = "{{ route('user.deleteOperative', ['temp', 'director']) }}";
          @endif
